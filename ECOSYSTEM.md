...
- **Media Playback**: Unified `lib/modules/media/` module — one mixed paginated library from `GET /v1/assets` (audio, video, avatar/image, and attachment; subtitle/thumbnail sidecars excluded), signed stream URLs from `GET /v1/assets/:id/playback` at play time for A/V (cached in `MediaService` until near `expires_at`, ~60s), display titles via API `title` / `displayTitle`, background audio with mini player, inline video via `better_player` (built-in overflow for playback speed and subtitles; quality/audio-track menus disabled), mixed next/previous across audio and video (skipping non-playable rows), and `children.subtitles[]` / playback `media.subtitles[]` (SRT) for closed captions and synced lyrics. Image taps open an in-app preview; attachments open externally. SRT bodies load through `MediaService.fetchSubtitleBody` (session URL cache). Per-item and bulk **Download all missing** offline saves use `MediaDownloadService` (A/V via playback URL; image/attachment via asset URL). Playback prefers the offline decrypted cache when a download is `ready`.
