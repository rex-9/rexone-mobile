...
| **Media**            | `Asset`                                                                                                                                | Unified media metadata (`name`, human-friendly `title`, optional `description`, `storage_key` for Garage/S3/Cloudinary/Local — user objects under `user/{user_id}/`, platform objects under `admin/`; format, size_bytes, original_size_bytes, compressed_size_bytes, compression_ratio, compression_passes, status enum: `pending`/`processing`/`ready`/`optimal`, duration_secs, type, polymorphic `assetable_type`/`assetable_id`), and `parent_asset_id` for generated video thumbnails, uploaded covers, and `.srt` subtitle children on compressible video or audio parents. Nested child assets under **`attributes.children`**: **`thumbnail`** (poster image) and **`subtitles[]`** (SRT caption tracks — mobile video CC and audio synced lyrics with per-track picker). Mobile playback resolves time-limited signed URLs via **`GET /v1/assets/:id/playback`** (`delivery.url`, `media.subtitles[]`, `expires_at`). |
...
- **Media Playback**: Unified `lib/modules/media/` module — one mixed paginated playlist from `GET /v1/assets`, signed stream URLs from `GET /v1/assets/:id/playback` at play time (cached in `MediaService` until near `expires_at`, ~60s), display titles via API `title` / `displayTitle`, background audio with mini player, inline video via `better_player` (built-in overflow for playback speed and subtitles; quality/audio-track menus disabled; remote network streaming optimized with 100MB chunk disk caching via `CacheConfiguration` and low-latency connection stabilization via `BufferingConfiguration`), standard `AppPage` with unified `AppBar` across playlist and video player, mixed next/previous across audio and video, and **`children.subtitles[]`** / playback `media.subtitles[]` (SRT) with per-track selection for closed captions and synced lyrics. SRT bodies are loaded through `MediaService.fetchSubtitleBody` (local file or signed CDN URL via `GetConnect` + `UrlHelper` headers) and cached by URL for the session so video/audio players do not re-download on every play. Video feeds prefetched SRT text into better_player as memory subtitle sources. Playback prefers the offline decrypted cache when a download is `ready`.
- **Offline Media Downloads & Drift SQLite**: Per-item AES-GCM sandbox downloads (`MediaDownloadService` + `MEDIA_OFFLINE_ENCRYPTION_KEY`; salt `rexone_mobile_offline_v1`). Fully backed by a local **Drift (SQLite)** database (`rexone_offline`, documented in [`docs/CLIENT_DATABASE.md`](docs/CLIENT_DATABASE.md)) mirroring the backend polymorphic `assets` schema (`assetable_type`, `assetable_id`, `parent_asset_id`).
  - **Offline Playlist Mode**: In offline mode or when network fails, the playlist queries local Drift SQLite. If empty, it displays a clear empty alert ("No downloaded videos to view in offline mode. Connect to the internet to stream or download videos to enjoy offline."). If 1 or 2 items are downloaded, only those items appear and play smoothly with offline thumbnails and subtitles. Reconnection auto-reloads the full catalog.
  - **Local-First Playback**: Downloaded media always streams from offline local storage even when online, conserving bandwidth and eliminating buffering.
  - **Single Icon-Space UX & Storage Transparency**: Trailing playlist control strictly occupies one icon-space across all states (`none`, `queued`/`downloading` with pause inside, `paused` with resume inside, `ready` with `'x'` remove icon, `failed` with retry icon). Active playing state is surfaced via artwork overlay and colored title. Displays human-readable size before download (`4.2 MB`), progress while downloading (`Downloading 1.2 MB / 4.2 MB (28%)`), and confirmation dialog with exact freed storage on deletion (`Remove (4.2 MB)`).
  - **Background Transfer**: Max 2 concurrent transfers. Files live under `Application Support/media_offline/` (`.enc` media, `.sub.enc` subtitles, `.tmp` parts, `decrypted_cache/`). Android progress via `background_downloader` group notification. iOS: Live Activity progress (`MediaDownloadWidget`, App Group `group.com.rexone.mobile`) plus complete/fail local notification. Pause/Resume on the Live Activity uses `MediaDownloadControlIntent` (`openAppWhenRun: false`) → App Group ActionStore → MethodChannel `rexone/media_download_live_activity` → `MediaDownloadNotificationService`. Platform plugins stay in `PushNotiService`; download UX owns `MediaDownloadNotificationService`.
...
| **Media: Offline Download Pause/Resume**                                  |      N/A      |         N/A          |            ✅            |
| **Media: iOS Live Activity Download Progress**                            |      N/A      |         N/A          |            ✅            |
