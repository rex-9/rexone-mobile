...
- **Offline Media Downloads**: Per-item AES-GCM sandbox downloads (`MediaDownloadService` + `MEDIA_OFFLINE_ENCRYPTION_KEY`; salt `rexone_mobile_offline_v1`). State machine: `none → queued → downloading ⇄ paused → processing → ready | failed`. Pause/resume via playlist controls and iOS Live Activity; long-press cancels in-flight/paused work; remove ready copies after confirm. Max 2 concurrent transfers. Files live under `Application Support/media_offline/` (`.enc` media, `.sub.enc` subtitles, `.tmp` parts, `decrypted_cache/`). App-only playback; logout cancels transfers and wipes the offline library. Not DRM — casual obfuscation. Android progress via `background_downloader` group notification. iOS: Live Activity progress (`MediaDownloadWidget`, App Group `group.com.rexone.mobile`, `live_activities` keyed by asset id with `title` / `progress` / `paused`) plus complete/fail local notification. Pause/Resume on the Live Activity uses `MediaDownloadControlIntent` (`openAppWhenRun: false`) → App Group ActionStore → MethodChannel `rexone/media_download_live_activity` → `MediaDownloadNotificationService` → `pauseDownload` / `resumeDownload`. Target rule: `LiveActivitiesAppAttributes` is **widget-only**; Intent + ActionStore ship in **Runner + widget**. Platform plugins stay in `PushNotiService`; download UX owns `MediaDownloadNotificationService`.
